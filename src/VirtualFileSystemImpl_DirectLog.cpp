...
VirtualFileSystemImpl_DirectLog::VirtualFileSystemImpl_DirectLog(
    ...
    try {
        ...
        mFps = static_cast<float>(videoInfo.fps);
...
size_t VirtualFileSystemImpl_DirectLog::generateFrame(
    ...
    auto task = [this, entry, pos, len, dst, result]() {
        try {
            ...
            // Extract RGB data from video frame
...
bool VirtualFileSystemImpl_DirectLog::convertRGBToDNG(
    ...
    // TODO: Implement proper RGB to DNG conversion
    // For now, create a placeholder DNG structure
    // This needs to:
    // 1. Create TIFF/DNG header
    // 2. Write image data (either mosaiced or as linear RGB)
    // 3. Apply log encoding
    // 4. Leave AsShot neutral and color matrices empty
    // 5. Set proper DNG tags
    
    spdlog::warn("RGB to DNG conversion not yet fully implemented for frame {}", frameNumber);
    
    // Placeholder: just copy RGB data for now
    dngData = rgbData;
