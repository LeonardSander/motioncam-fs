...
namespace bw64 {
  ...
  class Chunk {
   public:
    ...
    virtual void write(StreamWrapper& stream) const = 0;
  ...
  class UnknownChunk : public Chunk {
   public:
    ...
    UnknownChunk(StreamWrapper& stream, uint32_t id, uint64_t size) {
      chunkId_ = id;
      data_.resize(size);
    ...
    void write(StreamWrapper& stream) const override {
        stream.write(data_);
    }
  ...
  class iXmlChunk : public Chunk {
   public:
    static uint32_t Id() { return utils::fourCC("iXML"); }

      iXmlChunk(const std::string& axml) {
      std::copy(axml.begin(), axml.end(), std::back_inserter(data_));
    }

    uint32_t id() const override { return iXmlChunk::Id(); }
    uint64_t size() const override { return data_.size(); }

    /*
     * @brief Write the iXmlChunk to a stream
     */
    void write(StreamWrapper& stream) const override {
        stream.write(data_);
    }
  ...
  class AudioId {
   public:
     ...
     */
    void write(StreamWrapper& stream) const {
