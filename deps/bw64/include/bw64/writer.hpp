...
namespace bw64 {
  ...
  class Bw64Writer {
   ...
   void init(uint16_t channels, uint32_t sampleRate, uint16_t bitDepth, std::vector<std::shared_ptr<Chunk>> additionalChunks) {
      writeRiffHeader();
    ...
    Bw64Writer(const char* filename, uint16_t channels, uint32_t sampleRate, uint16_t bitDepth,
      ...
      fileStream_ = std::unique_ptr<FileStreamWrapper>(new FileStreamWrapper(filename, true));

      init(channels, sampleRate, bitDepth, additionalChunks);
    }
      
    Bw64Writer(const int fd,
               uint16_t channels,
               uint32_t sampleRate,
               uint16_t bitDepth,
               std::vector<std::shared_ptr<Chunk>> additionalChunks) {

      fileStream_ = std::unique_ptr<FileStreamWrapper>(new FileStreamWrapper(fd, true));
    
      init(channels, sampleRate, bitDepth, additionalChunks);
    }

    Bw64Writer(std::vector<uint8_t>& output,
               uint16_t channels,
               uint32_t sampleRate,
               uint16_t bitDepth,
               std::vector<std::shared_ptr<Chunk>> additionalChunks) {

        fileStream_ = std::unique_ptr<MemoryStreamWrapper>(new MemoryStreamWrapper(output));
