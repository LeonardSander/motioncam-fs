...
if [[ -n "$FUSE_LIB" ]]; then
  ...
  # macFUSE is linked by an absolute Homebrew/system path. Point every Mach-O
  # image in the bundle at the copy shipped in Contents/Frameworks instead.
  while IFS= read -r -d '' file; do
    while IFS= read -r dependency; do
      case "$dependency" in
        */libfuse.2.dylib)
          install_name_tool -change "$dependency" "@rpath/libfuse.2.dylib" "$file"
          ;;
      esac
...
if [[ -n "$LEFTOVER_MISSING" ]]; then
  echo "Missing @rpath entries remain:" >&2
